...
CREATE PROCEDURE [Staging].[Transform_AFR_ASXCallOptions_Daily]
AS
BEGIN
...
(
	SELECT [RunID]
		  ...
		  --,ROW_NUMBER()
	  FROM [PARM_Staging].[Staging].[AFR_ASXCallOptions_Daily_Raw]
	  WHERE OptionSeriesMonth NOT IN ('', 'CALL OPTIONS')
)
	...
	INSERT INTO [PARM_Staging].[Staging].[AFR_ASXCallOptions_Daily_Transformed]
	SELECT
		A.RunID,
		A.SystemID,
		A.FileID,
